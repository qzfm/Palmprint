...
def test(model, batch_size):
    ...
    classNumel = len(set(iddb_train)) # 600
...
if __name__== "__main__" :
    ...
    parser.add_argument("--epoch_num", type=int, default=1) # 训练的轮次
    ...
    parser.add_argument("--id_num", type=int, default=378, # 手掌id的总数
    ...
    parser.add_argument("--test_interval", type=str, default=100) # 每隔 100 个epoch测试一次
    parser.add_argument("--save_interval", type=str, default=50)  # 每隔 50  个epoch保存一次

    ##Training Path
    parser.add_argument("--train_set_file", type=str, default='./data/train_Tongji.txt') # 训练集标签文件路径
    parser.add_argument("--test_set_file", type=str, default='./data/test_Tongji.txt') # 测试集标签文件路径

    ##Store Path
    parser.add_argument("--des_path", type=str, default='./results/checkpoint/') # 模型权重保存路径
    parser.add_argument("--path_rst", type=str, default='./results/rst_test/') # 测试结果保存路径
